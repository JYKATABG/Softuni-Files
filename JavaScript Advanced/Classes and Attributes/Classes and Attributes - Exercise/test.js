class Company {
    ...
    bestDepartment() {
       let bestDepartmentAvg = Object.entries(this.departments).sort(([depNameOne,depInfoOne], [depNameTwo,depInfoTwo]) => {
            return depInfoTwo.avgSalary - depInfoOne.avgSalary
        })[0]
        let sortEmployee = bestDepartmentAvg[1].employees.sort((a,b) => {
            return b.salary - a.salary || a.name.localeCompare(b.name);
        })
        let buff = `Best Department is: ${bestDepartmentAvg[0]}\n`
        buff += `Average salary: ${bestDepartmentAvg[1].avgSalary}\n`
